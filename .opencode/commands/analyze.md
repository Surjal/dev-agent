...
Use the `task` tool with agent `dev-agent-researcher` to investigate the following request without modifying any files:
...
this project -- run the one-time Playwright/Obsidian setup check in `.opencode/docs/first-run-setup.md`, then
...
`.opencode/docs/obsidian-memory.md` for the project-detection algorithm and exact paths
