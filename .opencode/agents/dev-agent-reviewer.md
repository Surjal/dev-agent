...
- NEVER modify application files. Your `edit`/`write` permissions are denied at the tool layer -- findings go in your report, not into the code. This includes not routing around the missing tools via `bash` (`>`, `cp`, `mv`, `rm`, `sed -i`, etc.).
...
Any Critical Issue, or any `FAIL` in Security Verdict / Performance Verdict, forces `CHANGES REQUIRED`.
